...
def link(pl_file, label):
    return f'<a href="{at(pl_file)}" {L}>{label}</a>'
...
ARTICLES = [
 ...
 a("blog-robot-z-ai-rozmawiajacy-po-polsku.html", "2026-07-14", "KI · Interaktion",
   ...
   [("Technik", "Wie es funktioniert",
     ["Sprachmodelle im Hintergrund, Sprachsynthese für die Ausgabe, ein Mikrofon für die Eingabe. Der Roboter versteht Fragen auf Deutsch und antwortet in Echtzeit."]),
...
def build_article(art):
    ...
    ld = json.dumps({
        ...
        "dateModified": f"{art['date']}T08:00:00+02:00", "inLanguage": "de-AT",
    ...
    breadcrumb = json.dumps({
        ...
        "itemListElement": [
            ...
            {"@type": "ListItem", "position": 2, "name": "Blog", "item": f"{DOMAIN}/{at('blog.html')}"},
...
{_("nav_html")()}
  <nav class="crumbs" aria-label="Brotkrümel" style="max-width:1200px; margin:0 auto; padding:calc(var(--s8) + 48px) var(--s5) 0; font-size:0.78rem; letter-spacing:0.02em;">
    <a href="index.html" style="color:var(--text-3); text-decoration:none;">Startseite</a> <span aria-hidden="true" style="color:var(--text-3);">›</span> <a href="{at('blog.html')}" style="color:var(--text-3); text-decoration:none;">Blog</a> <span aria-hidden="true" style="color:var(--text-3);">›</span> <span style="color:var(--text-2);">{art['cat']}</span>
  </nav>
{_("mobile_menu")()}
  <section class="hero">
...
def related_articles(art):
    ...
    out = []
    for r in picks:
        out.append(f'        <a href="{at(r["pl_file"])}" style="display:flex; flex-direction:column; gap:4px; '
...
def make_cards(gen_globals):
    ...
    def cards(n):
        ...
        <a href="{at(art['pl_file'])}" class="tile__link">Artikel lesen →</a>
      </div>""")
        return "\n".join(out)
    return cards
...
def build_hub():
    ...
    ld = json.dumps({
        ...
        "name": "33bots Blog", "url": f"{DOMAIN}/{out_file}", "inLanguage": "de-AT",
...
def build_feed():
    ...
    for art in ARTICLES:
      ...
      <link>{DOMAIN}/{at(art['pl_file'])}</link>
      <guid>{DOMAIN}/{at(art['pl_file'])}</guid>
...
Firmenfeiern in ganz Österreich.
...
## Wichtige Seiten
- Startseite: {DOMAIN}/
- Roboter mieten: {DOMAIN}/{at('wypozyczenie-robota.html')}
- Messen: {DOMAIN}/{at('oferta-targi.html')}
- Konferenzen und Galas: {DOMAIN}/{at('oferta-konferencje.html')}
- Tage der offenen Tür: {DOMAIN}/{at('oferta-dni-otwarte.html')}
- Blog: {DOMAIN}/{at('blog.html')}
"""
...
def build(gen_globals):
    ...
    for art in ARTICLES:
        write(at(art["pl_file"]), build_article(art))
    write(at("blog.html"), build_hub())
