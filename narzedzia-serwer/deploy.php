<?php
/**
 * 33bots.at — punkt wdrożeniowy po stronie serwera (wersja austriacka).
 *
 * Pobiera aktualną wersję strony z GitHuba po HTTPS i rozpakowuje ją
 * do katalogu, w którym leży ten plik. Kierunek jest odwrotny niż przy
 * FTP: to serwer sięga po zmiany, dzięki czemu nie trzeba wpuszczać
...
// Uwaga: katalog og/ MUSI trafić na serwer — każda strona wskazuje w nim
// swoją kartę Open Graph.
const POMIJANE_KATALOGI = ['.git', '.github', 'node_modules', '__pycache__', 'build', 'narzedzia-serwer'];
const POMIJANE_PLIKI    = ['.gitignore', '.deployignore', '_redirects', 'deploy.php'];
const POMIJANE_KONCOWKI = ['.py', '.md', '.HEIC', '.DNG'];
const POMIJANE_NAZWY    = ['tailwind.config.js', 'tw-input.css', 'package.json', 'package-lock.json'];
...
function czyPomijac(string $sciezka): bool
{
    ...
    if (preg_match('/^IMG_\d+\.(JPG|PNG|HEIC|DNG)$/i', $nazwa)) return true;
    return false;
}
