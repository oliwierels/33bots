<?php
/**
 * 33bots.de — punkt wdrożeniowy po stronie serwera.
 *
 ...
 * dokładnie tak samo jak na 33bots.pl i 33bots.lt — dzięki czemu nie trzeba
 * wpuszczać niczego z zewnątrz ani przechowywać hasła FTP poza hostingiem.
 *
 * INSTALACJA
 *   1. Wpisz własny token w stałej TOKEN poniżej (ten sam, co w sekrecie
 *      DEPLOY_TOKEN_DE w repozytorium na GitHubie).
 *   2. Wgraj ten plik do public_html domeny 33bots.de.
 ...
 *
 * `33bots` jest publiczne, więc tu zostaje pusty ciąg. Gdyby kiedyś stało się prywatne,
 * wystarczy fine-grained token z jednym uprawnieniem: Repository access -> tylko to
 * repozytorium, Permissions -> Contents: Read-only.
 */
const GITHUB_TOKEN = '';

const REPO      = 'oliwierels/33bots';
const GALAZ     = 'claude/identical-pages-germany-rg6dzh';
 ...
 * Katalog `og/` JEST wysyłany: obrazki Open Graph niemieckiej strony leżą tylko
 * w repozytorium. Katalog `build/` to materiały robocze i zostaje na miejscu.
 */
const POMIJANE_KATALOGI = ['.git', '.github', 'node_modules', '__pycache__', 'build', 'scripts', 'narzedzia-serwer', 'templates'];
...
$plikZip = tempnam(sys_get_temp_dir(), '33bots_de_') ?: sys_get_temp_dir() . '/33bots_de.zip';
...
$tymczasowy = sys_get_temp_dir() . '/33bots_de_rozpakowane_' . bin2hex(random_bytes(4));
